...
def envelope_correlation(data, combine='mean', orthogonalize="pairwise",
    ...
    for ei, epoch_data in enumerate(data):
        ...
        evaluate("data_conj_scaled / data_mag", out=data_conj_scaled)
        if log:
            evaluate("data_mag * data_mag", out=data_mag)
        ...
        for li, label_data in enumerate(epoch_data):
            if orthogonalize is False:  # the new code
                ...
                label_data_orth = evaluate("(label_data * data_conj_scaled).imag")
                ...
                if log:
                    evaluate("label_data_orth * label_data_orth", out=label_data_orth)
