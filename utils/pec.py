...
def envelope_correlation(data, combine='mean', orthogonalize="pairwise",
    ...
    for ei, epoch_data in enumerate(data):
		...
		'''
		FREDERIK Notes...
		Note that this is being done on the analytical signals, i.e. by multiplying with the (normalized) complex conjugate of signal X 
		they effectively remove matching phases (imaginary components) from signal Y.

		As far as your plots go, you should see a reduction in spatial correlation after the orthogonalization. However, 
		I’m not quite sure if you should prefer orthogonalized over standard correlation. Not all phase coupling / locking is because of spatial proximity.
		'''
