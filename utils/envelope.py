...
def envelope_correlation(data, combine='mean', orthogonalize="pairwise", verbose=None, seed=None):
    ...
    for ei, epoch_data in enumerate(data):
        ...
        #for li, label_data in enumerate(epoch_data):
            #print(li)
            #if li == 43521:
                #print(f'Label data type {type(label_data)}') Numpy array
                #print(f'Label data shape {label_data.shape}') (5001,)
...
#     corrs = manager.list()
#     for ei, epoch_data in enumerate(data):
#         pool.apply_async(compute_correlation, args=[ei, epoch_data, corrs, False])
#     pool.close()
#     pool.join()
